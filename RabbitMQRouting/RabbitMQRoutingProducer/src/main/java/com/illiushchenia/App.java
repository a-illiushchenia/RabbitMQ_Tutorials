...
public class App
{
    ...
    public void sendMessageToRabbitMQ(String[] strings) throws IOException, TimeoutException {
        ...
        String logType = getSeverity(strings);
        String message = getMessage(strings);

        channel.basicPublish(RABBIT_MQ_EXCHANGE_NAME, logType, DURABLE_MESSAGE, message.getBytes());
        System.out.println(" [x] Sent '" + logType + "':'" + message + "'");
